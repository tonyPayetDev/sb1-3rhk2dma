...
const QuestionSequence = ({ question, style, backgroundUrl }) => {
  ...
  const progress = interpolate(frame, [0, 210], [0, 100], {
    extrapolateRight: "clamp",
  });

  const showAnswer = frame > 170;
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: style.backgroundColor }}>
      ...
      >
        <h2 style={{ fontSize: 32, fontFamily: style.font, fontWeight: "bold" }}>
          {question.text}
        </h2>
        ...
        >
          <div
            style={{ width: `${progress}%`, height: "100%", backgroundColor: style.accentColor }}
          />
        </div>
      </div>
    </AbsoluteFill>
...
const QuizVideo = ({ questions, style }) => {
  return (
    <AbsoluteFill>
      {questions.map((question, index) => (
        <Sequence key={index} from={index * 210} durationInFrames={210}>
          <QuestionSequence
            question={question}
            style={style}
...
export function VideoGenerator({ questions, style, onComplete, onError }) {
  const totalDuration = questions.length * 210;
  const [isRendering, setIsRendering] = useState(false);

  const handleRenderVideo = async () => {
    try {
      setIsRendering(true);
      const baseURL = "https://dev.tonypayet.com";
      const response = await fetch(`${baseURL}/api/render`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ questions, style, duration: questions.length * 7 }),
      });
      if (!response.ok) throw new Error("Failed to render video");
      const data = await response.json();
      const url = `${data.downloadLink}?download=true`;
