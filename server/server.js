...
app.post("/api/render", (req, res) => {
  ...
  let responseSent = false; // Flag pour éviter l'envoi multiple de réponse

  exec(command, { maxBuffer: 1024 * 10000 }, (error, stdout, stderr) => {
    if (responseSent) return; // Ne rien faire si la réponse a déjà été envoyée
    ...
    // Réponse avec le lien de téléchargement
    responseSent = true;
    res.json({
      message: "Vidéo prête !",
      downloadLink: `http://localhost:5000/video.mp4`, // ⚡ URL dynamique
    });
  });
});
